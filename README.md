...
- A recorded pile can be re-marked manually by selecting Re-mark and clicking its true location on the drawing preview. Clicking Re-mark again cancels it.
- Piles added manually without drawing coordinates still appear in the summary pages.

## Local Data

Records are stored locally in the user's browser. The same link opened in another browser, phone, or another user's device starts with its own separate local data.
